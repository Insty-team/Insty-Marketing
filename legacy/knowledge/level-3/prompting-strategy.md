<!-- 원본: https://claude-code-playbook-nu.vercel.app/docs/level-3/prompting-strategy -->

# 프롬프트 전략 | Claude Code 플레이북
...
복잡한 작업에서 Claude가 바로 코드를 수정하지 않고, **먼저 탐색하고 계획을 세운 뒤 승인받아 구현**하도록 할 수 있습니다. 이것이 Plan Mode입니다.
...
Plan Mode:
  요청 → 코드베이스 탐색 → 계획 수립 → 사용자 승인 → 구현
```

Plan Mode에서 Claude는 **읽기 전용**으로 동작합니다. 파일을 읽고, 검색하고, 구조를 파악하지만 어떤 파일도 수정하지 않습니다. 계획이 승인된 후에야 구현을 시작합니다.

### 언제 사용하는가?

- 여러 파일에 걸친 대규모 변경
- 아키텍처 결정이 필요한 기능 추가
- 접근 방식이 여러 가지인 복잡한 작업
- 기존 코드 구조를 파악한 뒤 작업해야 할 때
...
```
> 인증 시스템을 OAuth2로 전환하려고 해. 먼저 계획을 세워줘.

> 이 리팩토링을 plan mode로 진행해줘.

> 코드를 수정하기 전에 먼저 현재 구조를 파악하고 방법을 제안해줘.
```

### 동작 흐름

1. **탐색**: 관련 파일을 읽고 구조 파악
2. **계획 수립**: 변경 대상 파일, 접근 방식, 고려사항을 정리
3. **사용자 승인**: 계획을 보여주고 피드백 반영
4. **구현**: 승인된 계획대로 코드 수정

### 실전 예시

```
> OAuth2 소셜 로그인을 추가하려고 해. plan mode로 진행해.

Claude:
[탐색 중] 현재 인증 구조를 파악합니다...
- src/auth/auth.service.ts — JWT 기반 인증
- src/auth/auth.controller.ts — 로그인/회원가입 엔드포인트
- src/users/user.model.ts — User 스키마

[계획]
1. src/auth/oauth.service.ts 생성 — Google/GitHub OAuth 처리
2. src/auth/auth.controller.ts 수정 — /auth/google, /auth/github 엔드포인트 추가
3. src/users/user.model.ts 수정 — socialProvider 필드 추가
4. 환경변수 추가 — GOOGLE_CLIENT_ID, GITHUB_CLIENT_ID 등

이 계획으로 진행할까요?
```

**Plan Mode + CLAUDE.md 조합**

CLAUDE.md에 "복잡한 기능은 항상 plan mode로 진행할 것"이라고 적어두면, Claude가 자동으로 계획부터 세우는 습관을 갖습니다.
...
**다음 챕터**: [Remote Control →](/docs/level-3/remote-control)

**이전**: [트러블슈팅](/docs/level-3/troubleshooting)
