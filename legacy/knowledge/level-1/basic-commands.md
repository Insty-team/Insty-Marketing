<!-- 원본: https://claude-code-playbook-nu.vercel.app/docs/level-1/basic-commands -->

# 기본 명령어

> 💡 **이 챕터에서 배우는 것**: 파일 작업, 코드 분석, 디버깅, Git 연동 등 자주 쓰는 명령 패턴

**전제 지식**: 첫 번째 실행을 완료하고 기본 인터페이스에 익숙해진 상태 필요
...
> 터미널에 이 에러가 났어: [에러 메시지] — 원인과 해결법 알려줘
...
```
> UserList 컴포넌트가 너무 자주 리렌더링되는 것 같아.
  원인을 찾고 최적화해줘

> 이 SQL 쿼리가 느려: [쿼리] — 최적화 방법을 제안해줘
```
...
- 파일 읽기 → 분석 → 수정 → 검증 순으로 단계적 진행
- `/model`로 작업에 맞는 모델 선택 (비용 최적화)
- Claude Code는 git, npm, python 등 터미널 명령도 직접 실행 가능
...
→ [Desktop 앱](/docs/level-1/desktop-app)
