...
from app.schemas.video import MetadataSuggestionResponse
...
class VideoMetadataService:
    ...
    def generate_metadata_for_video(self, video_uuid: UUID) -> MetadataSuggestionResponse:
        ...
        if video.analysis_status == "FAILED":
            raise APIException(ErrorCode.ANALYSIS_FAILED, details=["영상 분석이 실패한 상태입니다."])
        ...
        try:
            prompt = load_prompt("metadata_prompt.j2", {"transcript": transcript_text.strip()})
        except Exception as e:
            raise APIException(ErrorCode.INTERNAL_ERROR, details=[f"프롬프트 로딩 실패: {str(e)}"])

        metadata = self._generate_with_gpt(prompt)

        return MetadataSuggestionResponse(**metadata)
    ...
    def _generate_with_gpt(self, prompt: str) -> dict:
        try:
            ...
            raise APIException(ErrorCode.INVALID_INPUT, details=[
                f"GPT 응답 파싱 실패: {str(e)}",
                f"GPT 원문: {content}"
            ])