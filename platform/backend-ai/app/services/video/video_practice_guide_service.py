...
from app.utils.clean_gpt_text import process_gpt_response
...
class VideoPracticeGuideService:
    def __init__(self, db: Session):
        self.db = db
        self.video_repo = VideoCourseRepository(db)

    def suggest_practice_guide(self, video_uuid: UUID) -> str:
        ...
        try:
            prompt = load_prompt("practice_guide_prompt.j2", {
                "transcript": transcript_text.strip()
            })
        except Exception as e:
            raise APIException(ErrorCode.INTERNAL_ERROR, details=[f"프롬프트 로딩 실패: {str(e)}"])

        draft_markdown = self._generate_with_gpt(prompt)
    ...
    def _generate_with_gpt(self, prompt: str) -> str:
        try:
            ...
            markdown = process_gpt_response(raw)
            return markdown
